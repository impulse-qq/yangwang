...
<p align="center">
  <sub>12のAIエージェント（11の業務ロール＋1の互換ロール）が核心部各小队を構成：副团长が振り分け、中書省が立案、門下省が審査、尚書省が配分、各小队が実行。<br>CrewAIにはない<b>制度的レビューゲート</b>。AutoGenにはない<b>リアルタイムダッシュボード</b>。</sub>
</p>
...
<p align="center">
  ...
  <br>
  <sub>🎥 フルデモ：核心部各小队によるAIマルチエージェント協調</sub>
</p>
...
<p align="center">
  ...
  <br>
  <sub>勅令発布 → 副团长振り分け → 中書省立案 → 門下省審査 → 各小队実行 → 回报（30秒）</sub>
</p>
</details>
...
```
あなた（皇帝） → 副团长（振り分け） → 中書省（立案） → 門下省（審査） → 尚書省（配分） → 各小队（実行） → 回报
   团长              副团长               中書省           門下省            尚書省            各小队          回报
```

これは単なる比喩ではありません。AIのための**真の三権分立**です：

- **副团长（Crown Prince）** がメッセージを振り分け — 雑談は自動返信、実際の指令はタスク化
...
### 🏛️ 十二部エージェントアーキテクチャ
- **副团长**（Crown Prince）メッセージ振り分け — 雑談は自動返信、実際の指令はタスク作成
- **核心部**（中書省・門下省・尚書省）による統治
- **七部**（戸部・书记小队・战斗小队・审判小队・建设小队・人事小队・朝報）による実行
...
インストーラーが自動的に以下を行います：
- 全部署のワークスペースを作成（`~/.openclaw/workspace-*`、副团长/人事小队/朝報を含む）
...
| 部署 | エージェントID | 役割 | 専門分野 |
|------|---------------|------|----------|
| 👑 **副团长** | `vice` | 振り分け、要約 | 雑談検出、意図抽出 |
| 📜 **中書省** | `strategy` | 受領、立案、分解 | 要件定義、アーキテクチャ |
| 🔍 **門下省** | `review` | 監査、門番、拒否権 | 品質、リスク、基準 |
| 📮 **尚書省** | `dispatch` | 割当、調整、収集 | スケジューリング、追跡 |
| 💰 **戸部** | `finance` | データ、リソース、経理 | データ処理、レポート |
| 📝 **书记小队** | `scribe` | 文書、基準、報告書 | テクニカルライティング、APIドキュメント |
| ⚔️ **战斗小队** | `combat` | コード、アルゴリズム、チェック | 開発、コードレビュー |
| ⚖️ **审判小队** | `audit` | セキュリティ、コンプライアンス、監査 | セキュリティスキャン |
| 🔧 **建设小队** | `build` | CI/CD、デプロイ、ツール | Docker、パイプライン |
| 📋 **人事小队** | `hr` | エージェント管理、研修 | 登録、権限管理 |
| 🌅 **朝報** | `intel` | デイリーブリーフィング、ニュース | 定期レポート、要約 |

### 権限マトリクス

| 送信元 ↓ \ 送信先 → | 副团长 | 中書省 | 門下省 | 尚書省 | 各小队 |
|:---:|:---:|:---:|:---:|:---:|:---:|
| **副团长** | — | ✅ | | | |
| **中書省** | ✅ | — | ✅ | ✅ | |
| **門下省** | | ✅ | — | ✅ | |
...
```
皇帝 → 副团长振り分け → 中書省立案 → 門下省審査 → 割当 → 実行中 → ✅ 完了
                           ↑          │                       │
                           └── 却下 ──┘              ブロック ──
```
...
│   ├── review/                 #   門下省
│   ├── dispatch/               #   尚書省
│   ├── finance/ scribe/ combat/     #   戸部 / 书记小队 / 战斗小队
│   ├── audit/ build/         #   审判小队 / 建设小队
│   ├── hr/                #   人事小队
│   └── intel/                #   朝報
├── dashboard/
...
- [x] 副团长振り分けレイヤー（雑談 vs タスクの自動ルーティング）
...
| 使用例 | コマンド | 部署 |
|--------|---------|------|
| [競合分析](examples/competitive-analysis.md) | 「CrewAI vs AutoGen vs LangGraphを分析せよ」 | 中書省→門下省→戸部+战斗小队+书记小队 |
| [コードレビュー](examples/code-review.md) | 「このFastAPIコードのセキュリティ問題をレビューせよ」 | 中書省→門下省→战斗小队+审判小队 |
| [週次レポート](examples/weekly-report.md) | 「今週のエンジニアリングチームレポートを生成せよ」 | 中書省→門下省→戸部+书记小队 |

各ケースには以下が含まれます：完全なコマンド → 中書省の提案 → 門下省のフィードバック → 各小队の出力 → 最終レポート。
