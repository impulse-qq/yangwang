...
def test_dirty_title_cleaned():
    cmd_create('JJC-TEST-E2E-01',
        ...
        'Strategy', '策划部', '策划部长',
        '发布委托（自动预建）：全面审查/Users/bingsen/clawd/项目')
...
def test_pure_path_rejected():
    cmd_create('JJC-TEST-E2E-02', '/Users/bingsen/clawd/openclaw-sansheng-liubu/', 'Strategy', '策划部', '策划部长')
    assert _get_task('JJC-TEST-E2E-02') is None, "纯路径标题应被拒绝"


# ── TEST 3: 正常标题正常创建
def test_normal_title():
    cmd_create('JJC-TEST-E2E-03', '调研工业数据分析大模型应用方案', 'Strategy', '策划部', '策划部长', '副团长整理委托')
...
def test_flow_remark_cleaned():
    cmd_create('JJC-TEST-E2E-04', '调研工业数据分析大模型应用方案', 'Strategy', '策划部', '策划部长')
    cmd_flow('JJC-TEST-E2E-04', '副团长', '策划部', '委托传达：审查/Users/bingsen/clawd/xxx项目 Conversation blah')
...
def test_short_title_rejected():
    cmd_create('JJC-TEST-E2E-05', '好的', 'Strategy', '策划部', '策划部长')
    assert _get_task('JJC-TEST-E2E-05') is None, "短标题应被拒绝"


# ── TEST 6: 传达委托前缀剥离
def test_prefix_stripped():
    cmd_create('JJC-TEST-E2E-06', '传达委托：帮我写技术博客文章关于智能体架构', 'Strategy', '策划部', '策划部长')
    t = _get_task('JJC-TEST-E2E-06')
    assert t is not None, "任务应被创建"
    assert not t['title'].startswith('传达委托'), f"前缀应被剥离: {t['title']}"


# ── TEST 7: state 更新 + org 自动联动
def test_state_update():
    cmd_create('JJC-TEST-E2E-07', '测试状态更新与组织联动功能', 'Strategy', '策划部', '策划部长')
    cmd_state('JJC-TEST-E2E-07', 'Review', '方案提交监察部审议')
    t = _get_task('JJC-TEST-E2E-07')
    assert t is not None
    assert t['state'] == 'Review', f"state应为Menxia: {t['state']}"
    assert t['org'] == '监察部', f"org应为监察部: {t['org']}"


# ── TEST 8: done 完成
def test_done():
    cmd_create('JJC-TEST-E2E-08', '测试任务完成状态标记功能', 'Strategy', '策划部', '策划部长')
...
def test_done_not_overwritable():
    cmd_create('JJC-TEST-E2E-09', '测试已完成任务不可覆盖保护', 'Strategy', '策划部', '策划部长')
    cmd_done('JJC-TEST-E2E-09', '/tmp/output.md', '任务已完成')
    cmd_create('JJC-TEST-E2E-09', '试图覆盖已完成的任务标题', 'Strategy', '策划部', '策划部长')
