...
> 核心部各小队的骨架：十二部制 + 副团长分拣 + 实时看板 + 完整工作流。

- [x] 十二部制 Agent 架构（副团长 + 中书·门下·尚书 + 户礼兵刑工 + 人事小队 + 晨报官）
- [x] 副团长分拣层 —— 自动识别闲聊/指令，闲聊直接回复，指令提炼标题后转策划部
- [x] 严格权限矩阵 —— 谁能给谁发消息，白纸黑字
- [x] 公会大厅实时看板（10 个功能面板）
- [x] 任务全生命周期管理（创建 → 分拣 → 规划 → 审议 → 派发 → 执行 → 回报）
- [x] 任务叫停 / 取消 / 恢复
- [x] 战报系统（已完成委托自动归档 + 五阶段时间线）
...
- [x] 委托数据清洗 —— 标题/备注自动净化，脏数据拒绝入库
- [x] 重复任务防护 —— 已完成/已取消委托不可覆盖
...
### 🔗 看板适配器
- [ ] Notion 适配器 —— 把 Notion database 变成公会大厅看板
- [ ] Linear 适配器 —— Linear 项目同步到核心部各小队
- [ ] GitHub Issues 双向同步
- **难度**：⭐⭐⭐
...
- [ ] 一键安装核心部各小队 Skill Pack
- **难度**：⭐

### 📈 年度大考
- [ ] Agent 年度绩效报告（Token 总消耗、完成率、最复杂委托）
- [ ] 可视化年度复盘大屏
- **难度**：⭐⭐
