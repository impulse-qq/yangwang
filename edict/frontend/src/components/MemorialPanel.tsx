  ...
  return (
    <div>
      ...
      <div className="mem-list">
        {!mems.length ? (
          <div className="mem-empty">暂无战报 — 任务完成后自动生成</div>
        ) : (
          mems.map((t) => {
            const fl = t.flow_log || [];
            const depts = [...new Set(fl.map((f) => f.from).concat(fl.map((f) => f.to)).filter((x) => x && x !== '团长'))];
...
function MemorialDetailModal({
  ...
  for (const f of fl) {
    if (f.from === '团长') originLog.push(f);
    else if (f.to === '策划部' || f.from === '策划部') planLog.push(f);
    else if (f.to === '监察部' || f.from === '监察部') reviewLog.push(f);
    else if (f.remark && (f.remark.includes('完成') || f.remark.includes('回报'))) resultLog.push(f);
    else execLog.push(f);
  }
    ...
    <div className="modal-bg open" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          {renderPhase('监察审核', '🔍', reviewLog)}
          {renderPhase('小队执行', '⚔️', execLog)}
          {renderPhase('汇总回报', '📨', resultLog)}
