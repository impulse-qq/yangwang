...
| 部门 | Agent ID | 职责 | 擅长领域 |
|------|----------|------|---------|
| � **副团长** | `vice` | 消息分拣、需求整理 | 闲聊识别、委托提炼、标题概括 |
| 📜 **策划部** | `strategy` | 接令、规划、拆解 | 需求理解、任务分解、方案设计 |
| 🔍 **监察部** | `review` | 审议、把关、驳回 | 质量评审、风险识别、标准把控 |
| 📮 **调度部** | `dispatch` | 派发、协调、汇总 | 任务调度、进度跟踪、结果整合 |
| 💰 **财务小队** | `finance` | 数据、资源、核算 | 数据处理、报表生成、成本分析 |
| 📝 **书记小队** | `scribe` | 文档、规范、报告 | 技术文档、API 文档、规范制定 |
| ⚔️ **战斗小队** | `combat` | 代码、算法、巡检 | 功能开发、Bug 修复、代码审查 |
| ⚖️ **审判小队** | `audit` | 安全、合规、审计 | 安全扫描、合规检查、红线管控 |
| 🔧 **建设小队** | `build` | CI/CD、部署、工具 | Docker 配置、流水线、自动化 |
| 📋 **人事小队** | `hr` | 人事、Agent 管理 | Agent 注册、权限维护、培训 |
| 🌅 **晨报官** | `intel` | 每日晨报、新闻聚合 | 定时播报、数据汇总 |
