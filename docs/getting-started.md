...
> 从零开始，5 分钟搭建你的核心部各小队 AI 协同系统

---

## 第一步：安装 OpenClaw

核心部各小队基于 [OpenClaw](https://openclaw.ai) 运行，请先安装：
...
在 OpenClaw 中配置消息渠道（Feishu / Telegram / Signal），将 `vice`（副团长）Agent 设为委托入口。副团长会自动分拣闲聊与指令，指令类消息提炼标题后转发策划部。
...
# 添加飞书渠道（入口设为副团长）
openclaw channels add --type feishu --agent vice
```
...
任务流转路径：
```
收件 → 副团长分拣 → 中书规划 → 监察审核 → 已派遣 → 执行中 → 已完成
```
