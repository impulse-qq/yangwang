...
_BASE = (
    pathlib.Path(os.environ["EDICT_HOME"])
    if "EDICT_HOME" in os.environ
...
AGENT_POLICY = {
    ...
    "intel": {"role": "coordination", "commands": {"progress", "todo", "memory"}},
    "finance": {
        "role": "execution",
        "commands": {
            "progress",
...
def _check_permission(agent_id, cmd):
    ...
    if cmd not in policy["commands"]:
        _append_audit(
            None, agent_id, "permission_denied", cmd, None, f"{agent_id} 越权执行 {cmd}"
        )
...
def _sanitize_text(raw, max_len=80):
    """清洗文本：剥离文件路径、URL、Conversation 元数据、传达委托前缀、截断过长内容。"""
    t = (raw or "").strip()
...
def cmd_create(task_id, title, state, org, official, remark=None):
    ...
    clean_remark = _sanitize_remark(remark) if remark else f"发布委托：{title}"
...
if _edict_task_path.exists():
    ...
    _VALID_TRANSITIONS = {
        "Pending": {"Vice", "Cancelled"},
        "Vice": {"Strategy", "Cancelled"},
        "Strategy": {"Review", "Cancelled", "Blocked"},
        "Review": {"Assigned", "Strategy", "Cancelled"},
        "Assigned": {"Doing", "Next", "Blocked", "Cancelled"},
        "Next": {"Doing", "Blocked", "Cancelled"},
        "Doing": {"Review", "Done", "Blocked", "Cancelled"},
        "Review": {"Done", "Review", "Doing", "Cancelled", "PendingConfirm"},
...
HIGH_RISK_TRANSITIONS = {
    ("Review", "Done"),  # 完结任务 — 需监察部确认
    ("Doing", "Cancelled"),  # 执行中取消 — 需调度部确认
    ("Review", "Cancelled"),  # 审核中取消 — 需策划部确认
}
...
def cmd_state(task_id, new_state, now_text=None):
    ...
    def modifier(tasks):
        ...
        if allowed is not None and new_state not in allowed:
            log.warning(
                f"⚠️ 非法状态转换 {task_id}: {old_state[0]} → {new_state}（允许: {allowed}）"
            )
            rejected[0] = True
            return tasks
        ...
        if (old_state[0], new_state) in HIGH_RISK_TRANSITIONS:
            ...
            t["pending_confirm"] = {
                ...
                "confirm_by": CONFIRM_AUTHORITY.get(old_state[0], "dispatch"),
            }
    ...
    if rejected[0]:
        ...
        log.info(
            f"⏳ {task_id} 高风险操作 {old_state[0]}→{new_state}，进入 PendingConfirm 待确认"
        )
        _append_audit(
            task_id,
            _infer_agent_id_from_runtime(),
            ...
            old_state[0],
            new_state,
            f"需 {CONFIRM_AUTHORITY.get(old_state[0], 'dispatch')} 确认",
        )
    else:
...
def cmd_done(task_id, output_path="", summary=""):
    ...
    def modifier(tasks):
        ...
        if output_path:
            ...
            if p.exists():
                ts = datetime.datetime.fromtimestamp(p.stat().st_mtime).strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
    ...
    _append_audit(
        task_id, _infer_agent_id_from_runtime(), "done", None, output_path, summary
    )
...
def cmd_block(task_id, reason):
    ...
    _append_audit(
        task_id, _infer_agent_id_from_runtime(), "block", None, "Blocked", reason
    )
...
def cmd_confirm(task_id, action, reason=""):
    ...
    def modifier(tasks):
        ...
        if t.get("state") != "PendingConfirm":
            log.warning(
                f"⚠️ {task_id} 不在 PendingConfirm 状态 (当前: {t.get('state')})"
            )
            rejected[0] = True
            return tasks
...
def cmd_progress(task_id, now_text, todos_pipe="", tokens=0, cost=0.0, elapsed=0):
    ...
    _append_audit(
        task_id, _infer_agent_id_from_runtime(), "progress", None, None, clean
    )
...
def cmd_todo(task_id, todo_id, title, status="not-started", detail=""):
    ...
    def modifier(tasks):
        ...
        if status == "in-progress":
            existing_ip = [
                td
                for td in t["todos"]
                if td.get("status") == "in-progress"
                and str(td.get("id")) != str(todo_id)
            ]
            if existing_ip:
                log.warning(
    ...
    if rejected[0]:
        log.info(
            f"❌ {task_id} todo #{todo_id} → in-progress 被拒（已有进行中的 todo）"
        )
        _append_audit(
            task_id,
            _infer_agent_id_from_runtime(),
    ...
    _append_audit(
        task_id, _infer_agent_id_from_runtime(), "todo", todo_id, status, title
    )
...
def cmd_memory(agent_id, mem_type, content, source_task="", tags=""):
    ...
    entry = {
        ...
        "type": mem_type
        if mem_type in ("feedback", "experience", "preference")
        else "experience",
        "content": content,
    ...
    def modifier(data):
        ...
        if len(memories) > MAX_AGENT_MEMORIES:
            ...
            unpinned.sort(
                key=lambda m: (m.get("type") == "feedback", m.get("created_at", ""))
            )
...
def cmd_task_memo(task_id, agent_id, decisions, warnings=""):
    ...
    warning_list = (
        [w.strip() for w in warnings.split(",") if w.strip()] if warnings else []
    )
    ...
    _append_audit(
        task_id, agent_id, "task_memo", None, None, f"{len(decision_list)} decisions"
    )
...
def cmd_delegate(task_id, from_agent, to_agent, instruction, return_spec=""):
    ...
    path = (
        parent_delegation.get("delegation_path", [from_agent])
...
def cmd_delegate_result(sub_task_id, result_json):
    ...
    _append_audit(
        parent_id, to_agent, "delegate_result", sub_task_id, None, result_json[:100]
    )
...
if __name__ == "__main__":
    ...
    if cmd == "create":
        ...
        cmd_task_memo(args[1], args[2], args[3], args[4] if len(args) > 4 else "")
        ...
        cmd_delegate(
            args[1], args[2], args[3], args[4], args[5] if len(args) > 5 else ""
        )
